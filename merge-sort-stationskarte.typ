...
#let headline-box() = block(
  ...
  #grid(
    ...
    [
      ...
      #linebreak()
      #text(size: 14pt, weight: "bold")[Sortieren nach Merge Sort entdecken]
      #v(3pt)
    ...
    [
      #align(center)[
        #deckz.hand("8S", "4S", "2S", "7S", "3S", format: "mini", angle: 16deg, width: 3.5cm)
...
#let tree-cards(..cards) = align(center)[
  #deckz.line(..cards, format: "small", spacing: 2pt)
]

/*------------------------------------------------------------------*
* Baut rekursiv eine Folge von Kartenrückseiten auf. Diese wird für *
* das ausfüllbare Baumdiagramm genutzt, damit die Struktur sichtbar *
* bleibt, ohne die Lösungen der Teilstapel schon vorzugeben.        *
*------------------------------------------------------------------*/
#let back-cards(count) = {
  if count <= 0 {
    ()
  } else {
    back-cards(count - 1) + (deckz.back(format: "small"),)
  }
}

/*----------------------------------------------------------------*
* Rendert eine Gruppe aus Kartenrückseiten als Platzhalterknoten. *
* Die Anzahl der Rückseiten zeigt, wie viele Karten in diesem     *
* Teilstapel später eingetragen werden sollen.                    *
*----------------------------------------------------------------*/
#let tree-backs(count) = align(center)[
  #stack(
    dir: ltr,
    spacing: 2pt,
    ..back-cards(count),
...
*/    ])
  ...
  [
  #infobox([So führst du zwei Stapel zusammen (Merge-Schritt)], [
...
#infobox([Baumdiagramm (zum Ausfüllen)], [
  Trage beim Teilen nacheinander die passenden Stapel ein.
  Die Kartenrückseiten zeigen nur, wie viele Karten an jeder Stelle liegen.
  #align(center)[
    #merge-tree[
...
#grid(
  ...
  gutter: 10pt,
  [
    #infobox([Selbstkontrolle], [
      Am Ende liegen die Karten so:
      #v(3pt)
      #card-row("1S", "2S", "3S", "4S", "5S", "6S", "7S", "8S")
    ], fill: green)
  ],
  [
    #infobox([Zusatzfragen], [
      1. *Rekursion:* Erkläre in einem Satz, was ein "Basisfall" ist und warum er wichtig ist.
      2. *Vergleich:* Nenne je einen Vorteil von Merge Sort gegenüber Selection Sort - und einen Nachteil.
    ], fill: yellow)
  ],
)
